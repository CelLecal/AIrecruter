...
const CandidatesProfile: React.FC = () => {
  ...
  useEffect(() => {
    fetch(`http://localhost:3000/candidate-documents/${id}`)
      .then((res) => {
        if (!res.ok) throw new Error('Документы не найдены');
        return res.json();
      })
      .then((data) => {
  ...
  const getYearString = (age: number) => {
    ...
    if (remainder100 > 10 && remainder100 < 20) return cases[0];
    if (remainder10 === 1) return cases[1];
    if (remainder10 >= 2 && remainder10 <= 4) return cases[2];
    return cases[0];
  };
    ...
    <div className={styles.profileContainer}>
      <div className={styles.profileHeader}>
        ...
        <div className={styles.headerMeta}>
          <span>
            <i className="fa fa-map-marker-alt"></i> {candidate.city}
          </span>
          <span>
            <i className="fa fa-phone-alt"></i> {candidate.phone}
          </span>
          <span>
      ...
      <div className={styles.twoColumns}>
        <div className={styles.leftColumn}>
          ...
          <div className={styles.card}>
            <h3>
              <i className="fa fa-robot"></i> Рекомендации AI
            </h3>
            <p className={styles.recommendationText}>{candidate.ai_summary || 'Нет данных'}</p>
          ...
          <div className={styles.card}>
            <h3>
              <i className="fa fa-user"></i> Личные данные
            </h3>
          ...
          <div className={styles.card}>
            <h3>
              <i className="fa fa-id-card"></i> Водительские данные
            </h3>
            <div className={styles.infoRow}>
            ...
            <div className={styles.infoRow}>
              ...
              <span>
                {candidate.experience_years
                  ? `${candidate.experience_years} ${getYearString(candidate.experience_years)}`
                  : '—'}
              </span>
            </div>
            <div className={styles.infoRow}>
          ...
          <div className={styles.card}>
            <h3>
              <i className="fa fa-certificate"></i> Дополнительные сертификаты
            </h3>
          ...
          <div className={styles.card}>
            <h3>
              <i className="fa fa-briefcase"></i> Предпочтения и условия
            </h3>
            <div className={styles.infoRow}>
          ...
          <div className={styles.card}>
            <h3>
              <i className="fa fa-clock"></i> Доступности
            </h3>
            <div className={styles.infoRow}>
        ...
        <div className={styles.rightColumn}>
          ...
          <div className={styles.card}>
            <h3>
              <i className="fa fa-tasks"></i> Действия
            </h3>
            <div className={styles.actionButtons}>
              <button className={styles.actionPrimary}>Подготовить оформление →</button>
              <button className={styles.actionHr} id="AnalyzeAi" onClick={handleClick}>
          ...
          <div className={styles.card}>
            <h3>
              <i className="fa fa-file-alt"></i> Проверенные документы
            </h3>
            {docsLoading ? (
              <p>Загрузка документов...</p>
            ) : docs.length === 0 ? (
              <p className={styles.noDocs}>Документы не загружены</p>
            ) : (
                ...
                {docs.map((doc) => (
                  <div key={doc.id} className={styles.docItem}>
                    ...
                    <div className={styles.docStatus}>
                      <span className={getStatusClass(doc.upload_status)}>
          ...
          <div className={styles.card}>
            <h3>
              <i className="fa fa-history"></i> История
            </h3>
            <div className={styles.historyList}>
              <div className={styles.historyItem}>
                <span>
                  <i className="fa fa-check-circle-o"></i> Документы проверены
                </span>
                <span className={styles.historyDate}>—</span>
              </div>
              <div className={styles.historyItem}>
                <span>
                  <i className="fa fa-check-circle-o"></i> Скрининг завершен
                </span>
                <span className={styles.historyDate}>—</span>
              </div>
              <div className={styles.historyItem}>
                <span>
                  <i className="fa fa-user-plus"></i> Кандидат добавлен
                </span>
          ...
          <div className={styles.card}>
            <h3>
              <i className="fa fa-chart-line"></i> Оценка рисков
            </h3>
            <div className={styles.riskRow}>
              <span>Соответствие требованиям</span>
              <span className={getScoreClassNum(candidate.hiring_score)}>
                {candidate.hiring_score ?? 0}%
              </span>
            </div>
            <div className={styles.riskRow}>
              <span>Общий риск</span>
              <span className={getRiskClass(candidate.risk_level)}>
                {candidate.risk_level || '—'}
              </span>
            </div>
          </div>
